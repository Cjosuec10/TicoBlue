...
@section('content')
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <div class="d-flex align-items-center">
                                                    @can('ver-comercio')
                                                        <a href="{{ route('comercios.show', $comercio->idComercio) }}" class="btn btn-info btn-sm me-1 w-80" title="Ver">
                                                            <i class="bi bi-eye"></i> Ver
                                                        </a>
                                                    @endcan

                                                    @can('editar-comercio')
                                                        <a href="{{ route('comercios.edit', $comercio->idComercio) }}" class="btn btn-warning btn-sm me-1 w-80" title="Editar">
                                                    ...
                                                    <div class="d-flex align-items-center form-check form-switch custom-switch-size ms-2">
                                                        <input
                                                            ...
                                                            type="checkbox"
                                                            role="switch"
                                                            data-id="{{ $comercio->idComercio }}"
                                                            id="switch-{{ $comercio->idComercio }}"
                                                            {{ $comercio->activo ? 'checked' : '' }}
                                                        />
                                                        <label class="form-check-label" for="switch-{{ $comercio->idComercio }}">
    ...
    document.querySelectorAll('.toggle-activation').forEach(switchElement => {
        switchElement.addEventListener('change', function () {
            const comercioId = this.getAttribute('data-id'); // Obtiene el ID del comercio del atributo data-id
            ...
            fetch(`/comercios/${comercioId}/toggle-activation`, {
                ...
                body: JSON.stringify({ activo: isActive }) // Envia el estado activo/inactivo en el cuerpo de la solicitud
            })
