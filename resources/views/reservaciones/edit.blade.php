        ...
        <div class="card-body">
            <form action="{{ route('reservaciones.update', $reservacion->idReservacion) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="date" name="fechaInicio" id="fechaInicio" class="form-control" value="{{ old('fechaInicio', $reservacion->fechaInicio) }}" required>
                </div>

                <!-- Fecha de Fin -->
                <div class="mb-3">
                    <label for="fechaFin" class="form-label">Fecha de Fin</label>
                    <input type="date" name="fechaFin" id="fechaFin" class="form-control" value="{{ old('fechaFin', $reservacion->fechaFin) }}" required>
                </div>

                <!-- Nombre del Usuario -->
                <div class="mb-3">
                    <label for="nombreUsuarioReservacion" class="form-label">Nombre del Usuario</label>
                    <input type="text" name="nombreUsuarioReservacion" id="nombreUsuarioReservacion" class="form-control" value="{{ old('nombreUsuarioReservacion', $reservacion->nombreUsuarioReservacion) }}" required>
                </div>

                <!-- Correo del Usuario -->
                <div class="mb-3">
                    <label for="correoUsuarioReservacion" class="form-label">Correo del Usuario</label>
                    <input type="email" name="correoUsuarioReservacion" id="correoUsuarioReservacion" class="form-control" value="{{ old('correoUsuarioReservacion', $reservacion->correoUsuarioReservacion) }}" required>
                </div>

                <!-- Teléfono del Usuario -->
                <div class="mb-3">
                    <label for="telefonoUsuarioReservacion" class="form-label">Teléfono del Usuario</label>
                    <input type="text" name="telefonoUsuarioReservacion" id="telefonoUsuarioReservacion" class="form-control" value="{{ old('telefonoUsuarioReservacion', $reservacion->telefonoUsuarioReservacion) }}">
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select name="idComercio_fk" id="idComercio_fk" class="form-select" required>
                        @foreach($comercios as $comercio)
                            <option value="{{ $comercio->idComercio }}" {{ $reservacion->idComercio_fk == $comercio->idComercio ? 'selected' : '' }}>
                ...
                <div class="mb-3">
                    ...
                    <select name="idEvento_fk" id="idEvento_fk" class="form-select">
                        ...
                        @foreach($eventos as $evento)
                            <option value="{{ $evento->idEvento }}" {{ $reservacion->idEvento_fk == $evento->idEvento ? 'selected' : '' }}>
                                {{ $evento->nombreEvento }}
                            </option>
                        @endforeach
                    </select>
...
<div class="col-md-6">
    ...
    <select class="form-select" id="idUsuario_fk" name="idUsuario_fk" required>
        ...
        @foreach($usuarios as $usuario)
            <option value="{{ $usuario->idUsuario }}" {{ $usuario->idUsuario == $reservacion->idUsuario_fk ? 'selected' : '' }}>
                {{ $usuario->nombre }}
            </option>
        @endforeach
    </select>
                ...
                <div class="mb-3">
                    ...
                    <select name="idAlojamiento_fk" id="idAlojamiento_fk" class="form-select">
                        ...
                        @foreach($alojamientos as $alojamiento)
                            <option value="{{ $alojamiento->idAlojamiento }}" {{ $reservacion->idAlojamiento_fk == $alojamiento->idAlojamiento ? 'selected' : '' }}>
